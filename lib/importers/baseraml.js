...
RAML.prototype._mapEndpoint = function(resource, baseURI, pathParams) {
  ...
  for (var i in methods) {
    ...
    var method = methods[i];
		...
		endpoint.Description = method.description ? jsonHelper.stringify(method.description) : method.description;
  ...
  try {
    ...
    if (!_.isEmpty(this.data.protocols)) {
      ...
      for (var i in this.project.Environment.Protocols) {
				if (!this.project.Environment.Protocols[i]) continue;
