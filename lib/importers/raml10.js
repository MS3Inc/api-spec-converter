...
RAML10.prototype.mapRequestBody = function(methodBody) {
	...
	for (var i in methodBody) {
		...
		else if (mimeType.schema && !_.isEmpty(mimeType.schema) && mimeType.schema[0].length) {
...
RAML10.prototype.mapSchema = function(schemData) {
	...
	for (var i in schemData) {
        ...
        sd.Name = definition.displayName;
	...
	schemas.forEach(function(schema) {
		updatedSchemas.push(RAML10.prototype.removeTypePropertyKindFromSchema(schema));
	});
	
	return updatedSchemas;
};

RAML10.prototype.removeTypePropertyKindFromSchema = function(schema) {
	if (schema.definition.typePropertyKind) {
		delete schema.definition.typePropertyKind;
	}
	if (schema.definition && schema.definition.properties) {
		for (var i in schema.definition.properties) {
			schema.definition.properties[i] = RAML10.prototype.removeTypePropertyKindFromProperty(schema.definition.properties[i]);
		}
	}
	if (schema.definition && schema.definition.items) {
		schema.definition.items = RAML10.prototype.removeTypePropertyKindFromProperty(schema.definition.items);
	}
	return schema;
};

RAML10.prototype.removeTypePropertyKindFromProperty = function(property) {
	if (property.typePropertyKind) {
		delete property.typePropertyKind;
	}
	if (property.properties) {
		for (var i in property.properties) {
			property.properties[i] = RAML10.prototype.removeTypePropertyKindFromProperty(property.properties[i]);
		}
	}
	if (property.items) {
		property.items = RAML10.prototype.removeTypePropertyKindFromProperty(property.items);
	}
	return property;
};
