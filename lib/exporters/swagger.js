...
Swagger.prototype._constructSwaggerMethod = function(endpoint, parameters, responses, env) {
  ...
  if (
    _.isArray(consumes) &&
    endpoint.Consumes &&
    !_.isEqual(env.Consumes, consumes) &&
    !_.isEmpty(consumes)
  ) {
  ...
  if (
    _.isArray(produces) &&
    endpoint.Produces &&
    !_.isEqual(env.Produces, produces) &&
    !_.isEmpty(produces)
  ) {
  ...
  for (var type in securedByTypes) {
    ...
    switch (type) {
      ...
      case 'oauth2':
        var result = {};
        result[type] = [];
        if (securityDefinitions[type] && securityDefinitions[type].scopes && securityDefinitions[type].scopes.length) {
          var scopes = securityDefinitions[type].scopes.map(function(scope) { return scope.name; });
          result[type] = scopes;
        }
        security.push(result);
        break;
...
Swagger.prototype._mapQueryString = function(queryStringParams) {
  ...
  for (var paramName in queryStringParams.properties) {
    ...
    param.in = 'query';
    param.required =
      queryStringParams.required && queryStringParams.required.indexOf(param.name) >= 0;
    parameters.push(param);
  }
  return parameters;
};
...
function mapResponseBody(res, mimeType) {
  ...
  if (
    mimeType && mimeType !== '' && res.example && res.example !== '{}' && res.example.length > 2
  ) {
    item.examples = {};
...
Swagger.prototype._mapResponseBody = function(endpoint, env) {
  ...
  return endpoint.Responses.reduce(function(result, response) {
    ...
    result[
      response.codes && response.codes.length > 0 && parseInt(response.codes[0])
        ? response.codes[0]
        : 'default'
    ] = item;
...
Swagger.prototype._mapRequestBody = function(slRequestBody, requestTypes) {
  ...
  if (!jsonHelper.isEmptySchema(body)) {
    ...
    if (
      slRequestBody.body.match(regex) ||
      (!_.isEmpty(requestTypes) &&
        ['multipart/form-data', 'application/x-www-form-urlencoded'].indexOf(requestTypes[0]) !==
          -1)
    ) {
...
Swagger.prototype._mapSchema = function(schemas) {
  return schemas.reduce(function(definitions, schema) {
    ...
    var convertedDefinition = RAML.prototype.convertRefFromModel(jsonHelper.parse(schema.Definition));
    if (convertedDefinition.type && convertedDefinition.type != 'nil') {
      if (isTypeValid(convertedDefinition.type)) {
        _.merge(definition, convertedDefinition);
        mapExample(schema, definition);
      }
      else if (convertedDefinition.allOf) {
        definition.type = convertedDefinition.allOf[1].type;
        definition.properties = convertedDefinition.allOf[1].properties;
      }
      definitions[schema.NameSpace] = Swagger.prototype.removeTypePropertyKindFromSchema(definition);
    }
...
Swagger.prototype.removeTypePropertyKindFromSchema = function(definition) {
	if (definition.typePropertyKind) {
		delete definition.typePropertyKind;
	}
	if (definition && definition.properties) {
		for (var i in definition.properties) {
			definition.properties[i] = Swagger.prototype.removeTypePropertyKindFromProperty(definition.properties[i]);
		}
	}
	if (definition && definition.items) {
		definition.items = Swagger.prototype.removeTypePropertyKindFromProperty(definition.items);
	}
	return definition;
};

Swagger.prototype.removeTypePropertyKindFromProperty = function(property) {
	if (property.typePropertyKind) {
		delete property.typePropertyKind;
	}
	if (property.properties) {
		for (var i in property.properties) {
			property.properties[i] = Swagger.prototype.removeTypePropertyKindFromProperty(property.properties[i]);
		}
	}
	if (property.items) {
		property.items = Swagger.prototype.removeTypePropertyKindFromProperty(property.items);
	}
	return property;
};

function isTypeValid(type) {
	var validType = ['object', 'array', 'integer', 'long', 'float', 'double', 'string', 'byte', 'binary', 'boolean', 'date', 'dateTime', 'password'];
  ...
  for (var i in endpoint.traits) {
    ...
    for (var i in trait.responses) {
      var res = trait.responses[i],
        code = res.codes && res.codes.length > 0 && parseInt(res.codes[0])
          ? res.codes[0]
          : 'default';
...
Swagger.prototype._mapEndpoints = function(swaggerDef, env) {
  ...
  for (var i in endpoints) {
    ...
    var responses = _.assign(
      {},
      this._mapEndpointTraitResponses(endpoint),
      this._mapResponseBody(endpoint, env)
    );
    ...
    swaggerDef.paths[endpoint.Path][endpoint.Method] = this._constructSwaggerMethod(
      endpoint,
      parameters,
      responses,
      env
    ...
    if (endpoint.SecuredBy) {
      var security = this._mapEndpointSecurity(
        endpoint.SecuredBy,
        this.project.Environment.SecuritySchemes
      );
...
Swagger.prototype._mapTraitResponses = function(traits, mimeType) {
  var responses = {};
  ...
  for (var i in traits) {
    ...
    for (var i in trait.responses) {
      var res = trait.responses[i],
        ...
        responseName = stringHelper.computeTraitName(
          trait.name,
          res.codes && res.codes.length > 0 && parseInt(res.codes[0]) ? res.codes[0] : 'default'
        );
...
Swagger.prototype._export = function() {
  ...
  var responses = this._mapTraitResponses(this.project.Traits, _.get(env, 'Produces[0]'));
  if (!_.isEmpty(responses)) {
  ...
  swaggerDef.securityDefinitions = this._mapSecurityDefinitions(
    this.project.Environment.SecuritySchemes
  );
